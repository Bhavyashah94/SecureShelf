...
MAX_FRAMES = 100  # Increased for a 4-second temporal window
MAX_PEOPLE = 3    # Maintaining tracking for multiple people
    ...
    for label, cls in enumerate(CLASSES):
        ...
        for video_name in os.listdir(cls_path):
            if video_name.endswith(('.mp4', '.avi')):
                print(f"Extraction V3: {cls} -> {video_name}")
                seq = extract_v3(os.path.join(cls_path, video_name))
                X.append(seq)
                y.append(label)

    X, y = np.array(X), np.array(y)
    print(f"\nExtraction V3 Complete! Shape: {X.shape}") 
    np.save(os.path.join(SAVE_DIR, 'X_features_v3.npy'), X)
    np.save(os.path.join(SAVE_DIR, 'y_labels_v3.npy'), y)